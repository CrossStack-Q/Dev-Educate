...
function JobCard({ jobID, index, jobTitle, jobCompany, jobLocation, jobSalaryMin, jobSalaryMax, jobDatePost, jobApplicants }) {
    ...
    // Fetch job details when the jobID prop changes
    useEffect(() => {
        const fetchJobDetails = async () => {
            try {
                const response = await fetch(`http://13.202.240.58:8080/job?id=${jobID}`);
                if (!response.ok) {
    ...
    const formatJobPostDate = (dateString) => {
        ...
        const options = { year: 'numeric', month: 'long' }; // Format: "Month Year"
